...
const LabelsForm = props => {
  const labelLis = allLabels.map(label => (
    <li key={label} onClick={e => props.onClickLabel(e, label)}>
      <div className={`${label}`} data-id="1">
    ...
    <div id="add-options-labels-dropdown">
      ...
      <div className="content">
        ...
        <div className="labels-search-results">
          <ul className="label-list">{labelLis}</ul>
          <ul className="light-list">
            <li className="not-implemented">Create a new label</li>
            <hr />
            {/* <li className="toggleColorblind" onClick={props.onToggleColorblind}>
